...
p1.showMsg();           // 原型：我的名字叫Stone，今年28岁，工作是Software Engineer

p1.name = 'Sophie';
// 当访问person1.name时，需要读取它的值。因此就会在这个实例上搜索一个名为name的属性，这个属性确实存在，于是就返回它的值而不必再搜索原型了。
console.log(p1.name);   // Sophie（来自实例）
// 把它删除以后，就恢复了对原型中name属性的连接。因此，接下来再调用person1.name时，返回的就是原型中name属性的值了。
delete p1.name;
...
delete p1.showMsg;// delete只能删除实例属性，不能删除构造函数prototype属性
p1.showMsg();   // 原型：我的名字叫 Stone，今年28岁，工作是 Software Engineer
delete Person.prototype.showMsg;
// p1.showMsg();   // TypeError: p1.showMsg is not a function
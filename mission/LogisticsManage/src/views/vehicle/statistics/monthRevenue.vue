<template>
  <div class="app-container">
    <h5>昨日报价台计：
      <san class="red">{{ list.totalCost }}</san>
      ，昨日成本台计：
      <san class="red">{{ list.totalQuotation }}</san>
      ，昨日利润台计：
      <san class="red">{{ list.totalProfit }}</san>
    </h5>
    <el-table v-loading="listLoading" :key="listKey" :data="list.vehicleMonthOrderList" border fit highlight-current-row>
...
<script>
  import { vehicleMonthOrder } from '@/api/vehicle/statistics'
  import {
    PAGE_TOTAL,
    PAGE_NUM,
    PAGE_SIZE
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        vehicleMonthOrder(this.listQuery).then((res) => {
          this.list = res.data

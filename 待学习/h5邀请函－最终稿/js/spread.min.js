!function(window) {
    ...
    Opa.prototype = {
        init: function() {
            var len = 0;
            this.set("pageNode", document.getElementById(this.get("page")) || document.body),
        ...
        _parseNode: function() {
            ...
            for (var i = 0, len = pans.length; len > i; i++) {
                ...
                0 !== i ? (item.style[this.get("prefix") + "Transform"] = "scale(2)",
                item.style.transform = "scale(2)",
                item.style.opacity = 0) : (util.addClass(item, "current"),
        ...
        getNext: function(index, flag) {
            var pans = this.get("panNodes")
              , len = pans.length
              , next = 0;
            return next = 1 === flag ? index + 1 == len ? 0 : index + 1 : 0 > index - 1 ? len - 1 : index - 1,
            pans[next]
        },
        preLoad: function(currentIndex) {
            var pans = this.get("panNodes")
              , len = pans.length
              , ci = +currentIndex
              , nci = 0 > ci - 1 ? len - 1 : ci - 1
              , pci = ci + 1 > len - 1 ? 0 : ci + 1
              , lazys = null ;
            pans[nci] && (lazys = pans[nci].querySelector(".J_MiLazyLoad"),
            lazys && (pans[nci].innerHTML = lazys.innerHTML)),
            pans[pci] && (lazys = pans[pci].querySelector(".J_MiLazyLoad"),
            lazys && (pans[pci].innerHTML = lazys.innerHTML))
        },
        bindEvent: function() {
            function exchange(ev) {
                function moveHandler(ev) {
                    ...
                    disy = move.y - start.y,
                    Math.abs(disy) > 20 && (disy > 0 ? target.getAttribute("data-index") > 0 ? (next = _this.getNext.call(_this, index, -1),
                    ...
                    next.style[pre + "Transform"] = "scale(" + (2 - -disy / vh) + ")",
                    next.style.transform = "scale(" + (2 - -disy / vh) + ")",
                    next.style.opacity = -disy / vh) : next = null ),
                    ev.preventDefault()
                }
                function endHandler(ev) {
                    console.log(_this.get("circle"));
                    _this.get("circle") ? (posObj = ev.changedTouches ? ev.changedTouches[0] : ev,
                    move.y = posObj.clientY || posObj.pageY,
                    disy = move.y - start.y,
                    ...
                    Math.abs(disy) > _this.get("diff") ? (next.style[pre + "Transform"] = "scale(1)",
                    next.style.transform = "scale(1)",
                    next.style.opacity = 1,
                    target.style[pre + "Transform"] = "scale(2)",
                    ...
                    }, 600)) : (target.style[pre + "Transform"] = "scale(1)",
                    target.style.transform = "scale(1)",
                    target.style.opacity = 1,
                    next.style[pre + "Transform"] = "scale(2)",
                    next.style.transform = "scale(2)",
                    next.style.opacity = 0,
                    setTimeout(function() {
                        _this.set("prevent", !1),
                        util.removeClass(target, "has-transition"),
                        util.removeClass(next, "has-transition")
                    }, 600))) : (_this.set("prevent", !1),
                    target.getAttribute("data-index") == len - 1 && _this.get("pb") && (_this.get("pbTimer") && (clearTimeout(_this.get("pbTimer")),
                    _this.pbTimer = null ),
                    _this.get("pb").style.visibility = "visible",
                    _this.get("pb").style.zIndex = 100,
                    _this.get("pb").style.opacity = 1)),
                    page.removeEventListener(EVENTS.touchmove, moveHandler, !1),
                    page.removeEventListener(EVENTS.touchup, endHandler, !1)
                }
                ...
                page.addEventListener(EVENTS.touchmove, moveHandler, !1),
                page.addEventListener(EVENTS.touchup, endHandler, !1)
            }
            var _this = this
              , util = App.Utils
              , pre = (_this.get("current"),
            _this.get("prefix"))
              ...
              , len = pans.length;
            page.addEventListener(EVENTS.touchdown, exchange, !1),
        ...
        pbEvent: function() {
            ...
            function pbTouchEnd(ev) {
                ...
                vdis = ey - sy,
                Math.abs(vdis) > 50 && vdis > 0 && (pb.style.opacity = 0,
            ...
            return pb ? (pb.addEventListener(EVENTS.touchdown, pbTouchStart, !1),
            void pb.addEventListener(EVENTS.touchup, pbTouchEnd, !1)) : !1
        }
    }
}(window);

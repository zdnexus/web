...
(function () {
    ...
    function oa(a, b, c) {
        ...
        if (typeof b == "string")if (S(c))a[e](b, c); else {
            if (a && a.getAttribute)f = a.getAttribute(b)
        } else if (S(b) && typeof b == "object")for (d in b)a[e](d, b[d]);
        return f
    }
    ...
    function pc(a) {
        var b = "", c;
        for (c in a)b += qc(c) + ":" + a[c] +
            ";";
        return b
    }

    function Sa(a, b) {
        if (Lc)if (b && b.opacity !== Ya)b.filter = "alpha(opacity=" + b.opacity * 100 + ")";
        O(a.style, b)
    }
    ...
    function gd(a) {
        function b(l, h) {
            function t() {
                ...
                q(ta, function (s) {
                    ...
                    if (!s.visible && H.ignoreHiddenSeries)p = false;
                    if (p) {
                        var y, B;
                        if (!Q) {
                        ...
                        if (s.isCartesian) {
                            q(s.data, function (G) {
                                var I = G.x, ca = G.y;
                                if (F === null)F = K = G[nb];
                                if (Q)if (I > K)K = I; else {
                                    if (I < F)F = I
                                } else if (S(ca)) {
                                    if (y)B[I] = B[I] ?
                                    B[I] + ca : ca;
                                    G = B ? B[I] : ca;
                                    if (!fc)if (G > K)K = G; else if (G < F)F = G;
                                    if (y)$a[s.type][I] = {total: G, cum: G}
                                }
                            });
                            if (/(area|column|bar)/.test(s.type) && !Q)if (F >= 0) {
                                F = 0;
                                Rc = true
                            } else if (K < 0) {
                                K = 0;
            ...
            function D(k, p, s) {
                if (s) {
                    ...
                    if (T) {
                        y = ja;
                        G = Aa - Bb;
                        if (k < Y || k > Y + wa)I = true
                    } else {
                        k = Y;
                        B = Ma - vc;
                        if (y < ja || y > ja + pa)I = true
                    }
                    I || L.path(L.crispLine([ib, k, y, Na, B, G], s)).attr({
                        stroke: p,
                        "stroke-width": s
                    }).add(M)
                }
            ...
            function E(k, p, s, y, B, G, I) {
                ...
                if (G && Ta.enabled)if ((k = gc.call({
                        index: I,
                        isFirst: k == ba[0],
                        isLast: k == ba[ba.length - 1],
                        value: Oa && Oa[k] ? Oa[k] : k
                    })) || k === 0) {
                    p = p + Ta.x - (Cb && T ? Cb * hb *
                        (zb ? -1 : 1) : 0);
            ...
            function ka(k, p) {
                ...
                if (!p) {
                    p = [1, 2, 2.5, 5, 10];
                    if (h.allowDecimals === false)if (tb == 1)p = [1, 2, 5, 10]; else if (tb <= 0.1)p = [1 / tb]
                }
                for (var y = 0; y < p.length; y++) {
                    k = p[y];
                    if (s <= (p[y] + (p[y + 1] || p[y])) / 2)break
                }
                k *= tb;
                return k
            }
            ...
            function ua() {
                ...
                if (ea == pb && Pa < 5 * ea)V = [1, 2, 5];
                Z = ka(Pa / ea, V);
                var ub;
                V = new Date(N * ab);
                V.setMilliseconds(0);
                if (ea >= s)V.setSeconds(ea >= y ? 0 : Z * jb(V.getSeconds() / Z));
                if (ea >= y)V[Nc](ea >= B ? 0 : Z * jb(V[rc]() / Z));
                if (ea >= B)V[Oc](ea >= G ? 0 : Z * jb(V[sc]() /
                    Z));
                if (ea >= G)V[uc](ea >= ca ? 1 : Z * jb(V[Rb]() / Z));
                if (ea >= ca) {
                ...
                for (B = V[Rb](); s < U && k < wa;) {
                    ba.push(s);
                    if (ea == pb)s = $b(ub + k * Z, 0) / ab; else if (ea == ca)s = $b(ub, y + k * Z) / ab; else if (!p && (ea == G || ea == I))s = $b(ub, y, B + k * Z * (ea == G ? 1 : 7)); else s += ea * Z;
                    k++
                }
                ba.push(s);
            ...
            function xa() {
                ...
                if (h.startOnTick)N = k; else N > k && ba.shift();
                if (h.endOnTick)U = p; else U < p && ba.pop()
            }
            ...
            function ya() {
                if (!kb && !Oa) {
                    ...
                    Mb = Nb[nb];
                    if (p < Mb) {
                        for (; ba.length < Mb;)ba.push(fa(ba[ba.length - 1] + Pa));
                        hb *= (p - 1) / (Mb - 1)
                    }
                    if (S(k) && Mb != k)aa.isDirty = true
                }
            }
            ...
            function bb() {
                ...
                if (!Oa && !fc && S(N) && S(U)) {
                    k = U - N || 1;
                    if (!S(h.min) && !S(Ba) && Tc && (F < 0 || !Rc))N -= k * Tc;
                    if (!S(h.max) && !S(Sb) && Uc && (K > 0 || !Sc))U += k * Uc
                }
                Pa = Oa || N == U ? 1 : A(h.tickInterval, (U - N) * h.tickPixelInterval / La);
                if (!kb && !S(h.tickInterval))Pa = ka(Pa);
                wc = h.minorTickInterval === "auto" && Pa ? Pa / 5 : h.minorTickInterval;
                xa();
                hb = La / (U - N || 1);
                Nb || (Nb = {x: 0, y: 0});
                if (!kb && ba.length > Nb[nb])Nb[nb] =
                    ba.length;
                if (!Q)for (p in $a)for (s in $a[p])$a[p][s].cum = $a[p][s].total;
                if (!aa.isDirty)aa.isDirty = N != y || U != B
            }
            ...
            function Ga(k, p, s) {
                ...
                za(aa, "setExtremes", {min: k, max: p}, function () {
                    if (Oa) {
                        if (k < 0)k = 0;
                        if (p > Oa.length - 1)p = Oa.length - 1
                    }
                    Ba = k;
                    Sb = p;
                    s && l.redraw()
            ...
            function x() {
                ...
                if (B) {
                    p && q(ba, function (I, ca) {
                        if (ca % 2 === 0 && I < U)J(I, ba[ca + 1] !== Ya ? ba[ca + 1] : U, p)
                    });
                    q(yc, function (I) {
                        J(I.from, I.to, I.color)
                    });
                    if (wc && !Oa)for (B = N; B <= U; B += wc) {
                ...
                if (!l.hasRendered) {
                    if (y) {
                        s = Y + (da ? wa : 0) + Ua;
                        B = Aa - Bb - (da ? pa : 0) + Ua;
                        L.path(L.crispLine([ib, T ? Y : s, T ? B : ja, Na, T ? Ma - vc : s, T ? B : Aa - Bb], y)).attr({
                            stroke: h.lineColor,
                            "stroke-width": y,
                            zIndex: 7
                        }).add()
                    ...
                    if (!aa.axisTitle && k && k.text) {
                        ...
                        s = (T ? ja + pa : Y) + (T ? 1 : -1) * (da ? -1 : 1) * k.margin - (Lc ? parseInt(k.style.fontSize || 12, 10) /
                            3 : 0);
                        aa.axisTitle = L.text(k.text, T ? y : s + (da ? wa : 0) + Ua, T ? s - (da ? pa : 0) + Ua : y, k.style, k.rotation || 0, {
                            low: "left",
                            middle: "center",
                            high: "right"
                        }[k.align]).attr({zIndex: 7}).add()
                    }
                }
                aa.isDirty = false
            }
            ...
            function u(k) {
                q([yc, xc], function (p) {
                    for (var s = 0; s < p.length; s++)if (p[s].id == k) {
                        p.splice(s, 1);
                        break
                    }
                });
        ...
        function d(l) {
            ...
            function v(X) {
                ...
                if (R === false || !C)t(); else {
                    ...
                    if (x < 5)x = 5; else if (x + xa > Aa)x = Aa - xa - 5;
                    h(W(u - ua), W(x - ua))
                }
            }
            ...
            E.padding = 0;
            var Ga = L.g("tooltip").attr({zIndex: 8}).add(), Va = L.rect(ua, ua, 0, 0, l.borderRadius, J).attr({
                fill: l.backgroundColor,
                "stroke-width": J
            }).add(Ga).shadow(l.shadow), qb = L.text("", ka + ua, parseInt(E.fontSize, 10) + ka + ua).attr({zIndex: 1}).css(E).add(Ga);
        ...
        function e(l, h) {
            function t(x) {
                ...
                if (!x.pageX)x.pageX = x.clientX + (la.documentElement.scrollLeft || la.body.scrollLeft);
                if (!x.pageY)x.pageY = x.clientY + (la.documentElement.scrollTop || la.body.scrollTop);
                return x
            }
            ...
            function v(x) {
                ...
                q(Qa, function (C) {
                    ...
                    u[Q ? "xAxis" : "yAxis"].push({
                        axis: C,
                        value: R(da ? x.pageX - lb.x - Y : pa - x.pageY + lb.y + ja, true)
                    })
                });
                return u
            }
            ...
            function D(x) {
                ...
                if (C && C.tracker)(x = C.tooltipPoints[ma ? x.pageY - lb.y :
            ...
            function ka() {
                ...
                Ha.onmousedown =
                    function (u) {
                        ...
                        fa = u.pageX;
                        Ea = u.pageY;
                        if (jc && (Ga || Va))ya || (ya = L.rect(Y, ja, qb ? 1 : wa, X ? 1 : pa, 0).attr({
                ...
                Ha.onclick = function (u) {
                    ...
                    u = t(u);
                    u.cancelBubble = true;
                    if (!xa)if (C && oa(u.target, "isTracker")) {
        ...
        function f(l) {
            ...
            if (v)if (ma && h == "column")t = eb.bar; else if (!ma && h == "bar")t = eb.column;
            h = new t;
            h.init(w, l);
            if (!v && h.inverted)ma = true;
        ...
        function i() {
            ...
            if (h)for (v = t; v--;) {
                D = ta[v];
                if (D.options.stacking)D.isDirty = true
            }
            q(ta, function (J) {
                if (J.isDirty) {
                    J.cleanData();
                    J.getSegments();
                    if (J.options.legendType == "point")l = true
                }
            });
            Nb = null;
            if (jc) {
        ...
        function m() {
            var l = a.loading;
            if (!Db) {
                Db = Da(yb, {className: "highcharts-loading"}, O(l.style, {
                    left: Y + Ia,
                    top: ja + Ia,
                    width: wa + Ia,
                    height: pa + Ia,
        ...
        function n(l) {
            var h, t, v;
            for (h = 0; h < Qa.length; h++)if (Qa[h].options.id == l)return Qa[h];
            for (h = 0; h < ta.length; h++)if (ta[h].options.id == l)return ta[h];
            for (h = 0; h < ta.length; h++) {
                v = ta[h].data;
                for (t = 0; t < v.length; t++)if (v[t].id == l)return v[t]
            }
            return null
        }
        ...
        function Wa() {
            mb = H.renderTo;
            Hc = Fb + Ic++;
            if (typeof mb == "string")mb = la.getElementById(mb);
            mb.innerHTML = "";
            if (!mb.offsetWidth) {
                vb = mb.cloneNode(0);
            ...
            Ha = Da(yb, {
                ...
                " " + H.className : ""), id: Hc
            }, O({position: Yc, overflow: Gb, width: Ma + Ia, height: Aa + Ia, textAlign: "left"}, H.style), vb || mb);
        ...
        function Hb() {
            ...
            l = 2 * v + (H.shadow ? 8 : 0);
            if (v || D)L.rect(l / 2, l / 2, Ma - l, Aa - l, H.borderRadius, v).attr({
            ...
            H.plotBorderWidth && L.rect(Y, ja, wa, pa, 0, H.plotBorderWidth).attr({
                "class": "plot-border",
        ...
        var Xb, Jc, Tb, Cc, Ec, Db, pa, wa, Ob, Fc, Gc, lb, jc = H.showAxes, Qa = [], Nb, ta = [], ma, L, Bc, Xc, Dc, $c, ld = function (l) {
            ...
            function v(r) {
                for (var M = Ea.length, F = r.checkbox; M--;)if (Ea[M] == r) {
                    Ea.splice(M, 1);
                    break
                }
            ...
            function D(r) {
                ...
                if (!K) {
                    ...
                    if (!F && r.options && r.options.lineWidth)r.legendLine = L.path([ib, -ua - fa, 0, Na, -fa, 0]).attr({
                        "stroke-width": r.options.lineWidth,
                        zIndex: 2
                    }).add(aa);
                    if (F)M = L.rect(-ua - fa, -11,
                        ua, 12, 2).attr({
                            "stroke-width": 0,
                            zIndex: 3
                        }).add(aa); else if (r.options && r.options.marker && r.options.marker.enabled)M = L.symbol(r.symbol, -ua / 2 - fa, -4, r.options.marker.radius).attr(r.pointAttr[Ka]).attr({zIndex: 3}).add(aa);
                    r.legendSymbol = M;
                    h(r, r.visible);
            ...
            function J() {
                ...
                if (T || $a) {
                    nb += 2 * Va;
                    La += 2 * Va;
                    if (da)da.attr({height: La, width: nb}); else da = L.rect(0, 0, nb, La, E.borderRadius,
                        T || 0).attr({
                            stroke: E.borderColor,
                            "stroke-width": T || 0,
                            fill: $a || fb
                        }).add(aa).shadow(E.shadow)
                }
        ...
        Dc = function (l) {
            ...
            !l || l.resetSelection ? q(Qa, function (t) {
                ...
                var v = t.axis;
                if (w.tracker[v.isXAxis ? "zoomX" : "zoomY"])v.setExtremes(t.min, t.max, false)
            });
            i()
        };
        Jc = function (l) {
            var h = l.align, t = l.verticalAlign, v = l.x || 0, D = l.y || 0, J = {x: v || 0, y: D || 0};
            if (/^(right|center)$/.test(h))J.x = (Ma - l.width) / {right: 1, center: 2}[h] + v;
            if (/^(bottom|middle)$/.test(t))J.y = (Aa - l.height) / {bottom: 1, middle: 2}[t] +
                D;
            return J
        };
        Wa();
        ...
        cb(db, "resize", z);
        cb(db, "unload", Pb);
        if (Ja)for (Xb in Ja)cb(w, Xb, Ja[Xb]);
        w.options = a;
        w.series = ta;
        w.container = Ha;
        w.chartWidth = Ma;
    ...
    function ad(a) {
        ...
        for (b = e - 2; b >= 0; b--)this.y2[b] = this.y2[b] *
            this.y2[b + 1] + a[b]
    }
    ...
    if (!Ca && db.jQuery) {
        ...
        cb = function (a, b, c) {
            if (typeof b ==
                ...
                if (!a.addEvent)if (a.nodeName)a = $(a); else O(a, new Events);
                a.addEvent(b, c)
            }
        };
        wb = function (a, b, c) {
    ...
    sa = {
        ...
        title: {
            text: "Chart title",
            x: 0, y: 20, align: "center", style: {color: "#3E576F", fontSize: "16px"}
        },
        subtitle: {text: "", x: 0, y: 40, align: "center", style: {color: "#6D869F"}},
        plotOptions: {
            line: {
        ...
        labels: {style: {position: Vb, color: "#3E576F"}},
        legend: {
            enabled: true,
            align: "center",
            layout: "horizontal",
            ...
            itemCheckboxStyle: {position: Vb, width: "13px", height: "13px"},
            symbolWidth: 16,
            symbolPadding: 5,
        ...
        loading: {
            ...
            showDuration: 100,
            style: {position: Vb, backgroundColor: "white", opacity: 0.5, textAlign: "center"}
        },
        tooltip: {
            enabled: true,
            formatter: function () {
    ...
    var ic = {
        ...
        tickWidth: 1,
        title: {align: "middle", margin: 35, style: {color: "#6D869F", fontWeight: "bold"}},
        type: "linear"
    }, zc = ia(ic, {
        endOnTick: true,
        gridLineWidth: 1,
        ...
        startOnTick: true,
        tickWidth: 0,
        title: {margin: 40, rotation: 270, text: "Y-values"}
    }), kd = {labels: {align: "right", x: -8, y: 3}, title: {rotation: 270}}, jd = {
        labels: {align: "left", x: 8, y: 3},
        title: {rotation: 90}
    }, Vc = {labels: {align: "center", x: 0, y: 14}, title: {rotation: 0}}, id = ia(Vc, {labels: {y: -5}});
    Ca = sa.plotOptions;
    ra = Ca.line;
    Ca.spline = ia(ra);
    ...
    var Kb =
        function (a) {
            function b(j) {
                if (g = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]?(?:\.[0-9]+)?)\s*\)/.exec(j))f = [parseInt(g[1], 10), parseInt(g[2], 10), parseInt(g[3], 10), parseFloat(g[4], 10)]; else if (g = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(j))f = [parseInt(g[1], 16), parseInt(g[2], 16), parseInt(g[3], 16), 1]
            }
    ...
    hc = function (a, b, c) {
        ...
        for (var n in b)a = a.replace("%" + n, b[n]);
    ...
    ec.prototype = {
        init: function (a, b) {
            ...
            if (typeof a == "string") {
                c = a;
                if (g == "circle")c = {x: "cx", y: "cy"}[c] || c; else if (c == "strokeWidth")c = "stroke-width";
                ...
                } else if (c == "fill")b = j.color(b, f, c); else if (g == "circle")c = {
                        x: "cx",
                        y: "cy"
                    }[c] || c; else if (c == "translateX" || c == "translateY") {
                    this[c] = b;
                    this.updateTransform();
                    i = true
                } else if (c == "stroke")b = j.color(b, f, c); else if (c == "isTracker")this[c] = b;
                if (c == "strokeWidth")c = "stroke-width";
                if (md && c == "stroke-width" && b === 0)b = 1.0E-6;
                ...
                if (m && /^(width|height|visibility|x|y|d)$/.test(c))for (d = m.length; d--;)oa(m[d], c, b);
                if (c == "text")j.buildText(f, b); else i || oa(f, c, b)
            }
            return n
    ...
    Wb.prototype = {
        init: function (a, b, c) {
            ...
            for (g = c.length - 1; g >= 0; g--)a.removeChild(c[g]);
            q(b, function (j, i) {
                var m = 0;
        ...
        arc: function (a, b, c, d, e, f) {
            ...
            return this.createElement("g").attr(S(a) && {
                    "class": Fb +
                    a
                })
            ...
            if (a && a.linearGradient) {
                ...
                j = f.createElement(b).attr({
                    id: g, gradientUnits: "userSpaceOnUse", x1: c[0], y1: c[1], x2: c[2],
                    y2: c[3]
                }).add(f.defs);
            ...
            if (e || f != "left")a = O(a, {
                "text-anchor": {left: "start", center: "middle", right: "end"}[f],
    ...
    if (!bd) {
        var qd = gb(ec, {
            init: function (a, b) {
                ...
                if (b == "shape" || b == yb)d.push("left:0;top:0;width:10px;height:10px;");
                c.push(' style="', d.join(""), '"/>');
                if (b) {
                ...
                if (typeof a == "string") {
                    ...
                    if (m && /^(x|y|r|start|end|width|height|innerR)/.test(c)) {
                        ...
                        e = d.length;
                        for (z = []; e--;)z[e] = typeof d[e] == "number" ? W(d[e] * 10) - 5 : d[e] == "Z" ? "x" : d[e];
                        d = z.join(" ");
                        f.path = d;
                        if (n)for (e = n.length; e--;)n[e].path = d;
                        z = true
                        ...
                        if (c ==
                            "y" && f.tagName == "SPAN" && f.lineHeight)d -= f.lineHeight;
                        g[{x: "left", y: "top"}[c]] = d
                        ...
                        c = "strokeweight";
                        if (typeof d == "number")d += Ia
                    } else if (c == "fill")if (j == "SPAN")g.color = d; else {
                    ...
                    if (n && c == "visibility")for (e = n.length; e--;)n[e].style[c] =
                        d;
                    if (c == "text")f.innerHTML = d; else if (!z)if (la.documentMode == 8)f[c] = d; else oa(f, c, d)
                }
                return na
            }, clip: function (a) {
        ...
        Kc.prototype = ia(Wb.prototype, {
            isIE8: qa.indexOf("MSIE 8.0") > -1, init: function (a, b, c) {
                ...
                return O(e, {
                    members: [], element: {style: {left: a, top: b, width: c, height: d}}, getCSS: function (f) {
                ...
                var b;
                if (a)b = {className: Fb + a, "class": Fb + a};
    ...
    Zb.prototype = {
        init: function (a, b) {
            ...
            if (typeof a == "number" || a === null)b.y = a; else if (typeof a == "object" && typeof a.length != "number") {
                O(b, a);
                b.options = a
        ...
        firePointEvent: function (a, b, c) {
            ...
            a || (a = Ka);
            if (!(b.selected && a != xb || d[a] && d[a].enabled === false || a && (g || f && !e.enabled)))if (a && !b.graphic) {
    ...
    Ra.prototype = {
        ...
        translate: function () {
            for (var a =
                ...
                if (o !== null)i.plotY = e.translate(o, 0, 1);
        ...
        setTooltipPoints: function (a) {
            ...
            q(e, function (m, o) {
                ...
                for (j = m.high = e[o + 1] ? jb((m.plotX + (e[o + 1] ? e[o + 1].plotX : f)) / 2) : f; g <= j;)i[d ? f - g++ : g++] = m
            });
            b.tooltipPoints = i
        },
        onMouseOver: function () {
            ...
            if (a.options.marker.enabled)for (g = c.length; g--;) {
                ...
                if (j.plotY !== Ya) {
                    ...
                    if (m)m.attr({
                        ...
                        y: f,
                        r: i
                    }); else j.graphic = d.renderer.symbol(A(j.marker && j.marker.symbol, a.symbol), e, f, i).attr(b).add(a.group)
                }
            }
        },
        ...
        getAttribs: function () {
            ...
            for (f = j.length; f--;) {
                ...
                b = g.options && g.options.marker ||
                    g.options;
                e = false;
                if (g.options)for (var n in o)if (b[o[n]])e = true;
                if (e) {
                    m = [];
                    c = b.states || {};
                    e = c[ob] = c[ob] || {};
                    if (!a.options.marker)e.color = Kb(e.color || g.options.color).brighten(e.brightness || d.brightness).get();
        ...
        drawDataLabels: function () {
            if (this.options.dataLabels.enabled) {
                ...
                if (g)g.empty(); else g = a.dataLabelsGroup = j.renderer.g(Fb + "data-labels").attr({
                    visibility: a.visible ? Yb : Gb,
                    zIndex: 4
                ...
                q(d, function (n) {
                    ...
                    o = e.align;
                    if (m == "column")b += {center: n.barW / 2, right: n.barW}[o] || 0;
                    if (f)n.dataLabel = j.renderer.text(f, b, c, e.style, e.rotation, o).attr({zIndex: 1}).add(n.group || g);
        ...
        drawGraph: function () {
            ...
            q(a.segments, function (z) {
                ...
                if (/^area/.test(a.type)) {
                    ...
                    for (ga = 0; ga < va; ga++)na.push(m[ga]);
                    if (b.stacking && a.type != "areaspline")for (ga = z.length - 1; ga >= 0; ga--)na.push(z[ga].plotX,
                        z[ga].yBottom); else na.push(z[z.length - 1].plotX, o, z[0].plotX, o, "z");
                    n = n.concat(na)
                }
            });
            if (n.length)a.areaPath = n;
            a.graphPath = e;
            if (d)d.attr({d: e}); else if (i)a.graph = c.path(e).attr({
                stroke: j,
                "stroke-width": i + Ia
            }).add(g).shadow(b.shadow);
            if (n.length) {
                d = A(b.fillColor, Kb(a.color).setOpacity(b.fillOpacity || 0.75).get());
                if (f)f.attr({d: n}); else a.area = a.chart.renderer.path(n).attr({fill: d}).add(a.group)
            }
        },
        render: function () {
        ...
        setState: function (a) {
            ...
            if (b.state != a) {
                ...
                if (!(e[a] && e[a].enabled === false)) {
                    if (a)c = e[a].lineWidth || c; else f && f.hide();
                    if (d)d.animate({"stroke-width": c}, a ? 0 : 500)
                }
            }
        },
        setVisible: function (a, b) {
            ...
            if (a)c.isDirty = o;
            f && f[i]();
            if (g)g[i](); else for (f = m.length; f--;) {
                g = m[f];
        ...
        drawTracker: function () {
            ...
            f = f && {cursor: f};
            if (e)e.attr({d: c}); else a.tracker = d.renderer.path(c).attr({
                isTracker: true,
                stroke: cd,
                fill: fb,
                "stroke-width": b.lineWidth + 2 * d.options.tooltip.snap,
    ...
    ad.prototype = {
        get: function (a) {
            ...
            for (var b = this.n - 1, c = 0; b - c > 1;) {
                var d = (b + c) / 2;
                if (this.xdata[jb(d)] > a)b = d; else c = d
            }
            b = jb(b);
            c = jb(c);
    ...
    qa = gb(Ra, {
        type: "spline", drawGraph: function () {
            ...
            q(a.segments, function (e) {
                ...
                if (f.length > 1)d = W(sb(b.plotWidth,
    ...
    var nc = gb(Ra, {
        ...
        translate: function () {
            ...
            q(b.series, function (va) {
                if (va.type == a.type)if (va.options.stacking) {
                    S(f) || (f = c++);
                    va.columnIndex = f
                } else va.columnIndex = c++
            });
            ...
            q(j, function (va) {
                ...
                if (S(H))va.trackerArgs = ia(va.shapeArgs, {height: 6, y: H})
            })
        },
        getSymbol: function () {
        },
        ...
        drawPoints: function () {
            ...
            q(a.data, function (f) {
                if (S(f.plotY)) {
                    d = f.graphic;
                    e = f.shapeArgs;
                    if (d)d.attr(e); else f.graphic = c[f.shapeType](e).attr(f.pointAttr[f.selected ? xb : Ka]).add(a.group).shadow(b.shadow)
                }
            })
        },
        drawTracker: function () {
            ...
            q(a.data, function (m) {
                ...
                if (e)e.attr(d); else m.tracker = c[m.shapeType](d).attr({
                    isTracker: f,
                    fill: cd,
                    visibility: a.visible ? Yb : Gb,
                    zIndex: 1
    ...
    qa = gb(Ra, {
        type: "pie", isCartesian: false, pointClass: qa, pointAttrToOptions: {
            stroke: "borderColor",
            ...
            q(this.data, function (g) {
                ...
                if (e)e.attr(f); else g.graphic = c.arc(f).attr(g.pointAttr[Ka]).add(g.group);
